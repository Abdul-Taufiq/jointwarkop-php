...
//query
$develop = query("SELECT * FROM develop WHERE id_dev = $id")[0];

//cek apakah tombol dipencet
if ( isset($_POST["submit"]) ) {

	//cek data berhasil
	if (ubahdevelop($_POST) > 0) {
		echo "
			<script>
...
<html>
<head>
	<title>Ubah Data Web Development</title>
...
<body>
  ...
  <div class="konten" align="center">
	<h1>UBAH DATA WEB DEVELOPMENT</h1>
	<form action="" method="post" enctype="multipart/form-data">
	<input type="hidden" name="id" value=" <?= $develop["id_dev"] ?> ">
	<table border="0" cellpadding="10" cellspacing="5" style="background-color: white; width: 40%; padding: 10px;">
		<tr>
			...
			<li style="margin: 10px 0;">
				...
				<input type="text" name="nim" id="nim" required autocomplete="off" style="margin: 10px; width: 70%; " value="<?=$develop["nim"]?>">
			</li>
			<li style="margin: 10px 0;">
				<label for="nama"> Nama : </label> <br>
				<input type="text" name="nama" id="nama" required autocomplete="off" style="margin: 10px; width: 70%; " value="<?=$develop["nama"]?>">
			</li>
			<li style="margin: 10px 0;">
				<label for="ttl"> TTL : </label> <br>
				<input type="text" name="ttl" id="ttl" required autocomplete="off" style="margin: 10px; width: 70%; " value="<?=$develop["ttl"]?>">
			</li>
			<li style="margin-top: 0;">
				<label for="alamat"> Alamat : </label><br>
				<textarea name="alamat" id="alamat" rows="3" cols="47" style="margin: 10px;" ><?=$develop["alamat"]?></textarea>
			</li>
			<li style="margin: 10px 0;">
			...
			<li style="margin: 10px 0;">
				...
				<input type="text" name="nohp" id="nohp" s autocomplete="off" style="margin: 10px; width: 70%;" value="<?=$develop["nohp"]?>">
			</li>
			<li style="margin: 10px 0;">
				<label for="gambar"> Gambar : </label><br>
				<input type="file" name="gambar" id="gambar" s autocomplete="off" style="margin: 10px; width: 70%;"  value="<?=$develop["gambar"]?>">
			</li>
